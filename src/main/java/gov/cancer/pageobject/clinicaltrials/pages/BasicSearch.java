...
import gov.cancer.pageobject.clinicaltrials.common.Delighter;
...
public class BasicSearch extends ClinicalTrialPageObjectBase {
  ...
  public BasicSearch(WebDriver browser, ClinicalTrialPageObjectBase decorator)
   ...
   *
   * @param cancerType String containing the exact cancer type name to be
   *                   selected. This must be the full name.
   */
  public void setExactCancerType(String cancerType) throws MalformedURLException, UnsupportedEncodingException {
    ...
    // CSS selector for the first list item, relative to the overall list. This
    // appears in the DOM
  ...
  /**
   * Places text in the "Cancer type or keyword" field, but doesn't select
   * anything from autocmplete.
   *
  ...
  /**
   * Places a value in the search form's "Age" field. Allows non-integer values in
   * order to allow tests of error handling.
   *
  ...
  /**
   * Gets the URL associated with the Advanced Search link in the "Search tips"
   * box.
   */
